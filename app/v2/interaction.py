...
class BaseInteraction:
    ...
    def __init__(self, session=_session, headers=_default_headers):
    ...
    def _get_auth_headers(self):
        return {"Authorization": "Bearer " + storage.get_access_token()}

    def _get_url(self, path):
        return "https://{subdomain}.amocrm.ru/api/v4/{path}".format(subdomain=SETTINGS.SUBDOMAIN, path=path)
    ...
    def _request(self, method, path, data=None, params=None, headers=None):
        ...
        raise exceptions.AmoApiException(
            "Wrong status {} ({})".format(response.status_code, response.text))
