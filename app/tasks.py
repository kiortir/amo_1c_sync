...
amo_limiter = BucketRateLimiter(
    backend, "amo_request_limit", limit=3, bucket=1_000
)
...
def setNote(note_text, lead_id):
    with amo_limiter.acquire():
        note_data = {"note_type": "common", "params": {"text": note_text}}
...
def setErrorStatus(lead_id: int, pipeline_id: int):
    with amo_limiter.acquire():
        ...
        if new_status is not None:
            Lead.objects.update(lead_id, {"status_id": new_status}),
...
def init_tokens(skip_error=False):
    try:
        ...
        data = {
            ...
            "code": amo_settings.auth_code,
            "redirect_uri": amo_settings.redirect_uri,
            "client_id": amo_settings.integration_id,
            "client_secret": amo_settings.secret_key,
        }
        try:
            response = httpx.post(
                "https://{}.amocrm.ru/oauth2/access_token".format(SUBDOMAIN),
                json=data,
            )
...
if base_settings.is_root:
    init_tokens()

    scheduler = BackgroundScheduler()
    scheduler.add_job(refresh_tokens.send, IntervalTrigger(hours=3))
    scheduler.start()
else:
    fetch_statuses()
